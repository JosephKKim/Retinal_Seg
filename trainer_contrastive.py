...
import ttach as tta
import torch.nn as nn
from utils.contrastive import negative_index_sampler, compute_reco_loss
...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for img, gt in tbar:
            ...
            # make label consist of bg and fg
            bg = torch.zeros_like(gt)
            bg = torch.where(gt == 0, 1, 0)
            vessel = torch.zeros_like(gt)
            vessel = torch.where(gt == 1, 1, 0)
            label = torch.cat([bg, vessel], dim = 1)
            ...
            bg_mean_map = torch.mean(1-pred_sigmoid, 1, keepdim = True)
            total_uncer_temp = -(mean_map * torch.log(mean_map + 1e-8))
            total_bg_uncer_temp = -(bg_mean_map * torch.log(bg_mean_map + 1e-8))
            total_uncer = total_uncer_temp
            total_uncer = total_uncer.sigmoid()
            total_bg_uncer = total_bg_uncer_temp.sigmoid()
            # normalize
            total_uncer = (total_uncer - total_uncer.min()) / (total_uncer.max() - total_uncer.min() + 1e-8)
            total_bg_uncer_norm = (total_bg_uncer - total_bg_uncer.min()) / (total_bg_uncer.max() - total_bg_uncer.min() + 1e-8)
            
            total_uncer = torch.cat([total_bg_uncer_norm, total_uncer], dim = 1)
            ...
            if self.CFG.amp is True:
                with torch.cuda.amp.autocast(enabled=True):
                    # rep [B, 256, 48, 48]
                    pre, latent_loss, rep = self.model(img)
    ...
    def _valid_epoch(self, epoch, dataset_name):
        ...
        with torch.no_grad():
            for img, gt in tbar:
                ...
                if self.CFG.amp is True:
                    with torch.cuda.amp.autocast(enabled=True):
                        predict, latent_loss, _ = self.model(img)
