'''
FR-UNet with swin module

'''
import torch
import torch.nn as nn
from .utils import InitWeights_He
from swin_u import HDRTransformer
...
class FR_UNet_Swin(nn.Module):
    def __init__(self,  num_classes=1, num_channels=1, feature_scale=2,  dropout=0.2, fuse=True, out_ave=True):
        super(FR_UNet_Swin, self).__init__()
        self.out_ave = out_ave
        filters = [64, 128, 256, 512, 1024]
        filters = [int(x / feature_scale) for x in filters] # [32, 64, 128, 256, 512]
        self.swin_block_12_128 = HDRTransformer(img_size = 12, embed_dim=128)
        # 왜 Embed dim을 바꿔줬지?
        self.swin_block_12_384 = HDRTransformer(img_size= 12, embed_dim= 384)
        
        # self.swin_block_24_64 = HDRTransformer(img_size = 24, embed_dim=64)
        
        
        # self.swin_block_148_128 = HDRTransformer(img_size = 148, embed_dim=128)
        # self.swin_block_148_384 = HDRTransformer(img_size= 148, embed_dim= 384)
    ...
    def forward(self, x):
        ...
        # row2
        # if self.training:
        # x2_2 = self.swin_block_24_64(x2_2)
        ...
        # # x3_1에 swin_u 적용
        # if self.training:
        # x3_1 resolution [256, 128 ,12, 12]
        x3_1 = self.swin_block_12_128(x3_1)
        # else:
        #     x3_1 = self.swin_block_148_128(x3_1)
        ...
        # row3: x_down20, x30, x_up40 cat한거에 swin_u적용
        
        row3_cat = torch.cat([x_down20, x30, x_up40], dim = 1)
        row3_cat = self.swin_block_12_384(row3_cat)
       
        _, x_up31 = self.block31(row3_cat)
