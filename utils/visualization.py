...
def visualize_dis_target(pred,folder_path):
    folder_path = os.path.join(folder_path, 'temp')
    for kk in range(pred.shape[0]):
        pred_edge_kk = pred[kk,:,:]
        pred_edge_kk = pred_edge_kk.detach().cpu().numpy().squeeze()
        pred_edge_kk *= 255.0
...
def visualize_gt(var_map,folder_path):
    ...
    for kk in range(10):
        ...
        cv2.imwrite(os.path.join(folder_path, name), pred_edge_kk)
...
def visualize_original_img(rec_img,folder_path):
    ...
    for kk in range(10):
        ...
        # since the original image is no more 3 channels comment out
        ...
        current_img = current_img.transpose(1,2,0)
        cv2.imwrite(os.path.join(folder_path, name), current_img)
