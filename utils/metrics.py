...
def get_metrics_notLogit(predict, target, threshold=None, predict_b=None):
    ...
    print(target.shape, predict_b.shape)
    print(predict_b)
...
def get_metrics_nosig(predict, target, threshold=None, predict_b=None):
    predict = predict.cpu().detach().numpy().flatten()
    
    if predict_b is not None:
        predict_b = predict_b.flatten()
        if torch.is_tensor(predict_b):
            predict_b = predict_b.cpu().detach().numpy().flatten()
    else:
        predict_b = np.where(predict >= threshold, 1, 0)
    if torch.is_tensor(target):
        target = target.cpu().detach().numpy().flatten()
