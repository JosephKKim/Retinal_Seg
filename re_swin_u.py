...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print("forward: ", x.shape) [512, 16, 256]
        ...
        k = k.reshape(B, nh, wh, ww, C)
        v = v.reshape(B, nh, wh, ww, C)

        # print(k.shape)
        # print(v.shape)
        
        up_k = torch.nn.functional.interpolate(k, mode = 'trilinear', scale_factor = (self.upscale,self.upscale,1))
        up_v = torch.nn.functional.interpolate(v, mode = 'trilinear', scale_factor = (self.upscale,self.upscale,1))

        

        # reshape
        B, nh, wh, ww, C = up_k.shape
        up_k = up_k.reshape(B, nh, -1, C)
        up_v = up_v.reshape(B, nh, -1, C)
        ...
        # TODO: fix relative positional embedding size
        # print(self.relative_position_index.shape)


        
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[0], (self.window_size[0] * self.upscale)**2 , -1)  # Wh*Ww,Wh*Ww,nH
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # print("before patch embed: ", x.shape) [8, 256, 32, 32]
...
class HDRTransformer(nn.Module):
    ...
    def forward(self, x1):
        ...
        x = self.conv_after_body(self.forward_features(x1) + x1)
