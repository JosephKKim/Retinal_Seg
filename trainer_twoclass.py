    ...
    def _train_epoch(self, epoch):
        ...
        for img, gt in tbar:
            ...
            self._metrics_update(
                # here we only count vessels
                *get_metrics(pre[:,1,...].unsqueeze(1), gt[:,1,...].unsqueeze(1), threshold=self.CFG.threshold).values())
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for img, gt in tbar:
                ...
                self.total_loss.update(loss.item())
                self._metrics_update(
                    # we only count vessels
                    *get_metrics(predict[:,1,...].unsqueeze(1), gt[:,1,...].unsqueeze(1), threshold=self.CFG.threshold).values())
                tbar.set_description(
