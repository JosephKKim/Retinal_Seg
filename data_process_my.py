...
def data_process(data_path, name, patch_size, stride, mode):
    save_path = os.path.join(data_path, f"{mode}_pro"+ '_' + str(patch_size)+ '_' +str(stride))
    ...
    for i, file in enumerate(file_list):
        if name == "DRIVE":
            ...
            # 여기서 Grayscale로 바꿔주는데 지금 우리 문제에서는 그러면 더 catch를 못할 수 있음
            # print(img.mode)
            # if img.mode != "RGB":
            #     img = img.convert("RGB")
            # print(img.size)
...
def get_patch(imgs_list, patch_size, stride):
    ...
    for sub1 in imgs_list:
        # w, h 방향으로 pad_w, pad_h 개의 0 padding.
        ...
        # print("size of patch shape ", sub2.shape)
    return image_list
