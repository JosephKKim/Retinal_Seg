...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for img, gt in tbar:
            ...
            #     self.scaler.scale(loss.mean()).backward(retain_graph=True)
            ...
            if self.CFG.amp is True:
                with torch.cuda.amp.autocast(enabled=True):
                    pre, latent_loss = self.model(img)
                    loss = self.loss(pre, gt) + latent_loss
                    w_diff_map = var_map * diff_map
                    uncer_loss = F.binary_cross_entropy_with_logits(w_diff_map, gt)
                    loss = self.loss(pre, gt) + latent_loss + uncer_loss 
                    # print("latent loss: ",latent_loss)
